  ...
  class AIKeyframeSynthesizer {
    ...
    async fetchKeyframes(prompt, opts = {}) {
      ...
      const requests = Array.from({ length: count }, (_, i) =>
        this._fetchOne(prompt, baseSeed + i * 7919, width, height, i, count, opts.signal)
          .then(img => { onProgress(++done, count); return img; })
          .catch(() => { onProgress(++done, count); return null; })
      );

      const settled = await Promise.all(requests);
      this.keyframes = settled.filter(Boolean);
      return this.keyframes;
    }

    async _fetchOne(prompt, seed, width, height, index, count, signal) {
      const res = await fetch(KEYFRAME_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          // something to actually move through instead of re-rolling the
          // same composition at a new seed.
          shotProgress: count > 1 ? index / (count - 1) : 0
        }),
        signal
      });
      ...
      return this._decode(data.dataUrl);
    }
