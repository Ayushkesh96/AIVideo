/**
 * HIGGSFIELD AI — EXACT HERO VISUAL ENGINES
 * High-fidelity procedural rendering for Hero cards matching official layout
 */

(function () {
  function initHeroVisuals() {
    // 1. Seedance 2.5 1080p Canvas
    ...
    if (cvSeedance) {
      ...
      function loopSeedance() {
        ...
        // Futuristic flight / bridge sunset
        ...
        for (let i = 0; i < 20; i++) {
          ...
          ctx.fillStyle = i % 2 === 0 ? 'rgba(0, 200, 255, 0.6)' : 'rgba(255, 120, 50, 0.5)';
          ctx.beginPath();
    ...
    // 2. Marketing Studio UGC Collage Canvas
    ...
    if (cvMarketing) {
      ...
      function loopMarketing() {
        ...
        // Warm aesthetic creator studio background
        const grad = ctx.createLinearGradient(0, 0, w, h);
        ...
        // 3 floating video frames simulator
        const cardW = w * 0.28;
        const cardH = h * 0.68;
    ...
    if (cvAdiliada) {
      ...
      function loopAdiliada() {
        ...
        // Dark fantasy ancient city battle
        const grad = ctx.createLinearGradient(0, 0, w, h);
        ...
        // Explosive embers & magic fire
        for (let i = 0; i < 25; i++) {
          const px = ((i * 41 + tick * 80) % w);
          const py = h - ((i * 59 + tick * 60) % h);
          ctx.fillStyle = i % 3 === 0 ? 'rgba(204, 255, 0, 0.7)' : 'rgba(255, 100, 30, 0.6)';
          ctx.beginPath();
    ...
    if (cvSpotlight) {
      ...
      function loopSpotlight() {
        ...
        for (let i = 0; i < 7; i++) {
          const headX = w * 0.15 + i * (w * 0.12) + Math.sin(tick + i) * 8;
          const headY = h * 0.55 + (i % 2) * 20;
          ctx.beginPath();
