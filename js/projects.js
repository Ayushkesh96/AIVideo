...
(function () {
    ...
    projectsData.forEach((proj, idx) => {
      ...
      card.innerHTML = `
        <div class="project-thumb-wrapper">
          ...
          <span class="project-badge-top">Template</span>
        </div>
        <div class="project-info">
          <h3 class="project-title">${proj.title}</h3>
          <div class="project-meta-row">
            <span style="color:var(--text-muted);">${proj.camera} • ${proj.aspect}</span>
            <span class="social-stat" style="color:var(--text-muted);">Use →</span>
          </div>
        </div>
      `;
      ...
      // The like button is gone with the invented counts it incremented —
      // there is no backend for it, so it only ever moved a made-up number.
  ...
  function openInspector(proj) {
    ...
    if (authorEl) authorEl.textContent = `${proj.camera} • ${proj.fps} fps • ${proj.aspect}`;
